...
* `reviewed` means the statement is identified and marked in source code.
* `tested#1` means the statement is tested via [Eclipse Paho Testing Utilities](https://github.com/eclipse/paho.mqtt.testing).
* `tested#2` means the statement is tested via unit test code.
...
|[MQTT-2.3.1-5]|`tested#1`|A PUBLISH Packet MUST NOT contain a Packet Identifier if its QoS value is set to 0.|
...
|[MQTT-3.1.2-4]|`reviewed` `tested#1`|If CleanSession is set to 0, the Server MUST resume communications with the Client based on state from the current Session (as identified by the Client identifier). If there is no Session associated with the Client identifier the Server MUST create a new Session. The Client and Server MUST store the Session after the Client and Server are disconnected.|
...
|[MQTT-3.1.2-7]|`reviewed` `tested#1`|Retained messages do not form part of the Session state in the Server, they MUST NOT be deleted when the Session ends.|
...
|[MQTT-3.1.2-10]|`reviewed` `tested#1`|The Will Message MUST be removed from the stored Session state in the Server once it has been published or the Server has received a DISCONNECT packet from the Client.|
|[MQTT-3.1.2-11]|`client spec`|If the Will Flag is set to 0 the Will QoS and Will Retain fields in the Connect Flags MUST be set to zero and the Will Topic and Will Message fields MUST NOT be present in the payload.|
...
|[MQTT-3.1.3-3]|`tested#1`|The Client Identifier (ClientId) MUST be present and MUST be the first field in the CONNECT packet payload.|
|[MQTT-3.1.3-4]|`tested#1`|The ClientId MUST be a UTF-8 encoded string as defined in Section 1.5.3.|
|[MQTT-3.1.3-5]|`tested#1`|The Server MUST allow ClientIds which are between 1 and 23 UTF-8 encoded bytes in length, and that contain only the characters "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ" |
...
|[MQTT-3.1.3-8]|`reviewed` `tested#1`|If the Client supplies a zero-byte ClientId with CleanSession set to 0, the Server MUST respond to the CONNECT Packet with a CONNACK return code 0x02 (Identifier rejected) and then close the Network Connection.|
|[MQTT-3.1.3-9]|`reviewed` `tested#1`|If the Server rejects the ClientId it MUST respond to the CONNECT Packet with a CONNACK return code 0x02 (Identifier rejected) and then close the Network Connection.|
...
|[MQTT-3.2.2-5]|`reviewed` `tested#1`|If a server sends a CONNACK packet containing a non-zero return code it MUST then close the Network Connection.|
|[MQTT-3.2.2-6]|`reviewed`|If none of the return codes listed in Table 3.1 – Connect Return code values are deemed applicable, then the Server MUST close the Network Connection without sending a CONNACK.|
...
|[MQTT-3.3.1-5]|`tested#1`|If the RETAIN flag is set to 1, in a PUBLISH Packet sent by a Client to a Server, the Server MUST store the Application Message and its QoS, so that it can be delivered to future subscribers whose subscriptions match its topic name.|
...
|[MQTT-3.3.5-1]|`tested#1`|The Server MUST deliver the message to the Client respecting the maximum QoS of all the matching subscriptions.|
...
|[MQTT-3.6.1-1]|`tested#1`|Bits 3,2,1 and 0 of the fixed header in the PUBREL Control Packet are reserved and MUST be set to 0,0,1 and 0 respectively. The Server MUST treat any other value as malformed and close the Network Connection.|
...
|[MQTT-3.8.4-1]|`tested#1`|When the Server receives a SUBSCRIBE Packet from a Client, the Server MUST respond with a SUBACK Packet.|
...
|[MQTT-3.8.4-5]|`tested#1`|The SUBACK Packet sent by the Server to the Client MUST contain a return code for each Topic Filter/QoS pair. This return code MUST either show the maximum QoS that was granted for that Subscription or indicate that the subscription failed.|
...
|[MQTT-3.9.3-1]|`tested#1`|The order of return codes in the SUBACK Packet MUST match the order of Topic Filters in the SUBSCRIBE Packet.|
|[MQTT-3.9.3-2]||SUBACK return codes other than 0x00, 0x01, 0x02 and 0x80 are reserved and MUST NOT be used.|
|[MQTT-3.10.1-1]|`tested#1`|Bits 3,2,1 and 0 of the fixed header of the UNSUBSCRIBE Control Packet are reserved and MUST be set to 0,0,1 and 0 respectively. The Server MUST treat any other value as malformed and close the Network Connection.|
...
|[MQTT-3.10.4-4]|`tested#1`|The Server MUST respond to an UNSUBSUBCRIBE request by sending an UNSUBACK packet. The UNSUBACK Packet MUST have the same Packet Identifier as the UNSUBSCRIBE Packet.|
|[MQTT-3.10.4-5]|`tested#1`|Even where no Topic Subscriptions are deleted, the Server MUST respond with an UNSUBACK.|
...
|[MQTT-3.14.4-3]|`tested#1`|On receipt of DISCONNECT the Server MUST discard any Will Message associated with the current connection without publishing it, as described in Section3.1.2.5.|
|**CHAPTER #4**|||
...
|[MQTT-4.6.0-6]|`tested#1`|When a Server processes a message that has been published to an Ordered Topic, it MUST follow the rules listed above when delivering messages to each of its subscribers. In addition it MUST send PUBLISH packets to consumers (for the same Topic and QoS) in the order that they were received from any given Client.|
|[MQTT-4.7.1-1]||The wildcard characters can be used in Topic Filters, but MUST NOT be used within a Topic Name.|
|[MQTT-4.7.1-2]|`tested#1`|The multi-level wildcard character MUST be specified either on its own or following a topic level separator. In either case it MUST be the last character specified in the Topic Filter.|
...
|[MQTT-4.7.3-1]|`tested#1`|All Topic Names and Topic Filters MUST be at least one character long.|
|[MQTT-4.7.3-2]|`tested#1`|Topic Names and Topic Filters MUST NOT include the null character (Unicode U+0000).|
|[MQTT-4.7.3-3]|`tested#1`|Topic Names and Topic Filters are UTF-8 encoded strings, they MUST NOT encode to more than 65535 bytes.|

...
|[MQTT-3.1.2-1]|`CODEC`| If the protocol name is incorrect the Server MAY disconnect the Client, or it MAY continue processing the CONNECT packet in accordance with some other specification. In the latter case, the Server MUST NOT continue to process the CONNECT packet in line with this specification.| 
|[MQTT-3.1.2-2]|`CODEC`| The Server MUST respond to the CONNECT Packet with a CONNACK return code 0x01 (unacceptable protocol level) and then disconnect the Client if the Protocol Level is not supported by the Server.| 
...
|[MQTT-3.1.2-6]|`reviewed`| If CleanSession is set to 1, the Client and Server MUST discard any previous Session and start a new one. This Session lasts as long as the Network Connection. State data associated with this Session MUST NOT be reused in any subsequent Session.| 
...
|[MQTT-3.1.2-10]|`reviewed`| The Will Message MUST be removed from the stored Session state in the Server once it has been published or the Server has received a DISCONNECT packet from the Client.| 
...
|[MQTT-3.1.2-18]|`client spec`| If the User Name Flag is set to 0, a user name MUST NOT be present in the payload.| 
...
|[MQTT-3.1.3-3]|`CODEC`| The Client Identifier (ClientId) MUST be present and MUST be the first field in the CONNECT packet payload.| 
|[MQTT-3.1.3-4]|`CODEC`| The ClientId MUST be a UTF-8 encoded string as defined in Section 1.5.3.| 
|[MQTT-3.1.3-5]|`CODEC`| "The Server MUST allow ClientIds which are between 1 and 23 UTF-8 encoded bytes in length, and that contain only the characters "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"|| 
...
|[MQTT-3.1.3-9]|`reviewed`| If the Server rejects the ClientId it MUST respond to the CONNECT Packet with a CONNACK return code 0x02 (Identifier rejected) and then close the Network Connection.| 
|[MQTT-3.1.3-10]|`CODEC`| The Will Topic MUST be a UTF-8 encoded string as defined in Section ‎1.5.3.| 
|[MQTT-3.1.3-11]|`CODEC`| The User Name MUST be a UTF-8 encoded string as defined in Section 1.5.3.| 
|[MQTT-3.1.4-1]|`CODEC`| The Server MUST validate that the CONNECT Packet conforms to section 3.1 and close the Network Connection without sending a CONNACK if it does not conform.| 
...
|[MQTT-3.1.4-5]|`reviewed`| If the Server rejects the CONNECT, it MUST NOT process any data sent by the Client after the CONNECT Packet.| 
|[MQTT-3.2.0-1]|`reviewed`| The first packet sent from the Server to the Client MUST be a CONNACK Packet.| 
...
|[MQTT-3.2.2-5]|`reviewed`| If a server sends a CONNACK packet containing a non-zero return code it MUST then close the Network Connection.| 
...
|[MQTT-3.3.1-12]|`reviewed`| If the RETAIN flag is 0, in a PUBLISH Packet sent by a Client to a Server, the Server MUST NOT store the message and MUST NOT remove or replace any existing retained message.| 

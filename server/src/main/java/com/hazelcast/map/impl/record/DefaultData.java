...
public final class DefaultData implements Data {
	...
	public DefaultData(byte[] data) {
		if (data != null && data.length > 0
				&& data.length < DATA_OFFSET) { throw new IllegalArgumentException(
						"Data should be either empty or should contain more than " + DefaultData.DATA_OFFSET
	...
	public int getPartitionHash() {
		if (hasPartitionHash()) { return Bits.readIntB(data, data.length - Bits.INT_SIZE_IN_BYTES); }
		return hashCode();
	}
	...
	@Override
	public int getType() {
		if (totalSize() == 0) { return SerializationConstants.CONSTANT_TYPE_NULL; }
	...
	private static boolean equals(byte[] data1, byte[] data2) {
		...
		if (data2.length != length) { return false; }

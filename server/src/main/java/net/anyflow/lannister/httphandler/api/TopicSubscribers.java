...
@HttpRequestHandler.Handles(paths = { "api/topicSubscribers" }, httpMethods = { "GET" })
public class TopicSubscribers extends HttpRequestHandler {
	...
	private String all() {
		try {
			return new ObjectMapper().writeValueAsString(TopicSubscriber.NEXUS.keySet().stream()
					.map(key -> TopicSubscriber.NEXUS.get(key)).collect(Collectors.toList()));
		}

...
@HttpRequestHandler.Handles(paths = { "api/topicSubscriptions" }, httpMethods = { "GET" })
public class TopicSubscriptions extends HttpRequestHandler {
	...
	private String all() {
		try {
			return new ObjectMapper().writeValueAsString(TopicSubscription.NEXUS.keySet().stream()
					.map(key -> TopicSubscription.NEXUS.get(key)).collect(Collectors.toList()));
		}
